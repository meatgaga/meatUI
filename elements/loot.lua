----------------------------------------------------------------------------------------
-- 拾取窗口 (基于Butsu by Haste)
----------------------------------------------------------------------------------------
if not SettingsCF.loot.loot_frame == true then return end
...
function Butsu:LOOT_OPENED(event, autoloot)
	...
	if(GetCVar("lootUnderMouse") == "1") then
		...
		self:ClearAllPoints()
		self:SetPoint("TOPLEFT", nil, "BOTTOMLEFT", SettingsDB.Scale(x - 40), SettingsDB.Scale(y + 20))
		self:GetCenter()
		self:Raise()
	end
...
--			slot:SetBackdropBorderColor(color.r, color.g, color.b)
	...
	close:SetPoint("TOPRIGHT", 0, SettingsDB.Scale(1))
	close:SetScript("OnClick", function(self) self:GetParent():Hide() end)

	self:SetWidth(SettingsDB.Scale(SettingsCF.loot.width))
	self.title:SetWidth(SettingsDB.Scale(SettingsCF.loot.width - 45))
	self.title:SetHeight(SettingsDB.Scale(SettingsCF.loot.font_size))
end
...
do
	...
	title:SetFont(SettingsCF.media.font, SettingsCF.loot.font_size, SettingsCF.media.font_style)
	title:SetJustifyH("LEFT")
	title:SetPoint("TOPLEFT", Butsu, "TOPLEFT", SettingsDB.Scale(7), SettingsDB.Scale(-7))
	Butsu.title = title
end
...
SettingsDB.CreateTemplate(Butsu, 0.6)
...
do
	...
	function addon.CreateSlot(id)
		...
		iconFrame:SetHeight(SettingsDB.Scale(SettingsCF.loot.icon_size))
		iconFrame:SetWidth(SettingsDB.Scale(SettingsCF.loot.icon_size))
		...
		icon:SetPoint("TOPLEFT", SettingsDB.Scale(1), SettingsDB.Scale(-1))
		icon:SetPoint("BOTTOMRIGHT", SettingsDB.Scale(-1), SettingsDB.Scale(1))
		frame.icon = icon

		local count = iconFrame:CreateFontString(nil, "OVERLAY")
		count:SetJustifyH("RIGHT")
		count:SetPoint("BOTTOMRIGHT", iconFrame, SettingsDB.Scale(-1), SettingsDB.Scale(2))
		...
		name:SetFont(SettingsCF.media.font, SettingsCF.loot.font_size, SettingsCF.media.font_style)
		...
		drop:SetPoint("TOPLEFT", 0, 0)
		drop:SetPoint("BOTTOMRIGHT", 0, 0)
		drop:SetAlpha(0.3)
		frame.drop = drop
	...
	function Butsu:AnchorSlots()
		...
		self:SetHeight(SettingsDB.Scale((shownSlots * (frameSize + 3)) + 30))
	end
end
