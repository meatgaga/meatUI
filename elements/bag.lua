...
local function Print (x)
	DEFAULT_CHAT_FRAME:AddMessage("|cffC495DDmeatUI:|r " .. x)
end
...
function Stuffing:SlotUpdate(b)
	...
	if not b.frame.lock then
		b.frame:SetBackdropBorderColor(0.3,0.3,0.3,1)
	end
...
function Stuffing:CreateBagFrame(w)
	...
	if w == "Bank" then
		...
		if numraid < 6 then
			...
			f:SetPoint ("BOTTOMRIGHT", UIParent, "BOTTOMRIGHT", -x, y+125)
		end
	end
	-- close button
...
function Stuffing:Layout(lb)
	...
	if lb then
		...
		f.editbox:SetFont(font, 12)
		f.detail:SetFont(fontn, 10)
		f.gold:SetFont(fontn, 10)

		f.detail:ClearAllPoints()
		f.detail:SetPoint("TOPLEFT", f, 13, -10)
	...
	f:SetWidth(cols * 31 + (cols - 1) * 4 + 13 * 2)
		...
		if bag_cnt > 0 then
			...
			if not (hide_soulbag == 1 and bagType == ST_SOULBAG) then
				...
				for j = 1, bag_cnt do
					...
					b.frame:SetBackdropColor(.1,.1,.1)
...
local function StuffingSlashCmd(Cmd)
	...
	elseif cmd == "purchase" then
		...
		if Stuffing.bankFrame and Stuffing.bankFrame:IsShown() then
			...
			Print(string.format("花費: %.2f 金", GetBankSlotCost() / 10000))
			Print("購買銀行背包欄位. (需要造訪銀行)")
		else
			Print("您需要先造訪您的銀行")
		end
	else
		Print("sort - " .. "將背包或銀行內的物品分類及排序")
		Print("stack - " .. "將背包或銀行內的不完整的物品堆疊重新堆疊")
		Print("purchase - " .. "購買銀行背包欄位. (需要造訪銀行)")
	end
end


function Stuffing:ADDON_LOADED(addon)
	if addon ~= "meatUI" then
		return nil
	end
