	...
	for index, value in ipairs(threatlist) do
		...
		if ( not bar ) then
			...
			if ( index == 1 ) then
				...
				bar:SetPoint("TOP", threatbars[index-1], "BOTTOM", 0, SettingsDB.Scale(-3))
			end

			bar.background = bar:CreateTexture("$parentBackground", "BACKGROUND")
			bar.background:SetAllPoints()
			bar.background:SetTexture(SettingsCF.media.blank)

			bar.backgdrop = CreateFrame("Frame", nil, bar)
			SettingsDB.CreateTemplate(bar.backgdrop)
			bar.backgdrop:SetFrameStrata("BACKGROUND")
			bar.backgdrop:SetPoint("TOPLEFT", SettingsDB.Scale(-1), SettingsDB.Scale(1))
			bar.backgdrop:SetPoint("BOTTOMRIGHT", SettingsDB.Scale(1), SettingsDB.Scale(-1))

			bar.textright = bar:CreateFontString("$parentTextRight", "ARTWORK")
			bar.textright:SetFont(SettingsCF.media.pixel_font, SettingsCF.media.pixel_font_size, SettingsCF.media.pixel_font_style)
			...
			bar.textleft:SetFont(SettingsCF.media.font, SettingsCF.media.font_size-1, SettingsCF.media.font_style)
			bar.textleft:SetJustifyH("LEFT")
			bar.textleft:SetPoint("LEFT", SettingsDB.Scale(2), SettingsDB.Scale(0.5))
			bar.textleft:SetPoint("RIGHT", bar.textright, "LEFT", SettingsDB.Scale(-1), SettingsDB.Scale(0.5))
...
local function OnEvent(self, event, ...)
	local unit = ...
	if ( event == "ADDON_LOADED" and unit == "meatUI" ) then
...
SlashCmdList["ThreatMeter"] = function(msg)
...
SLASH_ThreatMeter1 = "/tm"
SLASH_ThreatMeter2 = "/threatmeter"