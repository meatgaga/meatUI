----------------------------------------------------------------------------------------
-- ÈøÂú¼ÀË¾Í¼ÌÚÀ¸ÃÀ»¯ (by Darth Android)
----------------------------------------------------------------------------------------
if IsAddOnLoaded("Skinner") then return end
local buttonspacing = SettingsDB.Scale(2)
local borderspacing = SettingsDB.Scale(1)
...
local function SkinFlyoutButton(button)
	...
	button:SetWidth(SettingsDB.buttonsize + borderspacing)
	button:SetHeight(buttonspacing * 3 + borderspacing - 1)
	button.skin:SetWidth(SettingsDB.buttonsize + borderspacing)
...
local function SkinActionButton(button, colorr, colorg, colorb)
	SettingsDB.CreateTemplate(button, 0)
...
local function SkinFlyoutTray(tray)
	...
	for i,k in ipairs(buttons) do
		...
		if k.icon then
			...
			SettingsDB.CreateTemplate(k, 0.6)
	...
	tray:SetWidth(SettingsDB.buttonsize + buttonspacing * 2)
	tray:SetHeight((SettingsDB.buttonsize+buttonspacing) * numbuttons + buttonspacing)
...
function AddOn:PLAYER_ENTERING_WORLD()
	if select(2,UnitClass("player")) == "SHAMAN" then
		...
		bgframe:SetHeight(SettingsDB.buttonsize + buttonspacing * 2)
		bgframe:SetWidth(buttonspacing + (buttonspacing + SettingsDB.buttonsize) * 6)
