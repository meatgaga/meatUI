----------------------------------------------------------------------------------------
-- ±ã½Ý³ÔºÈºê (»ùÓÚcaelBuffet by Caellian)
----------------------------------------------------------------------------------------
...
function caelBuffet:PLAYER_REGEN_ENABLED()
	if dirty then self:Scan() end
end

function caelBuffet:BAG_UPDATE()
	dirty = true
	if not InCombatLockdown() then self:Scan() end
end
caelBuffet.PLAYER_LEVEL_UP = caelBuffet.BAG_UPDATE

function caelBuffet:Scan()
...
function caelBuffet:Edit(name, substring, food, pot, stone, shift)
